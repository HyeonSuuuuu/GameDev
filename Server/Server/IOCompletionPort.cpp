...
namespace IOCompletionPort
{
	...
	bool IOCompletionPort::BindandListen(const int32 port)
	{
	...
	{
		m_clientInfos.resize(maxClientCount);
	}
	...
	bool IOCompletionPort::SendMsg(ClientInfo* pClientInfo, char* pMsg, int len)
	{
		DWORD dwRecvNumBytes = 0;
